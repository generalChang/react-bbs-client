...
const CommunityUpdatePage = () => {
    ...
    axios
      ...
      .then(({ data }) => {
        if (data.success) {
          ...
          checkPermission(com.manager._id);
          setTitle(com.title);
  ...
  const onSubmit = (e) => {
    ...
    axios
      .post(`${requests.api_community}/${requests.community_update}`, body, {
        withCredentials: true,
      })
      .then(({ data: { success } }) => {
        if (success) {
          message.success("커뮤니티 수정에 성공했습니다.");
          navigate("/mypage", {
            replace: true,
          });
          return;
        } else {
          message.warning("커뮤니티 수정에 실패했습니다.");
          return;
        }
      })
      .catch((err) => {});
  ...
  const checkPermission = (manager) => {
    if (user.userData._id != manager) {
      message.warning("수정할 권한이 없습니다.");
      navigate("/", {
        replace: true,
      });
    }
